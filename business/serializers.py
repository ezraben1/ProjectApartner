...
class ApartmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Apartment
        fields = ['id', 'owner', 'address', 'description', 'size', 'rooms']
...
class RoomSerializer(serializers.ModelSerializer):
    ...
    renter_id = serializers.IntegerField(write_only=True, required=False)
    
    class Meta:
        model = Room
        fields = ['id', 'description','size', 'price_per_month',
                  'window', 'images',
                  'apartment_id', 'renter_id','contract']
...
class BillSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        """
        Check that the current user is the owner of the apartment.
        """
        user = self.context['request'].user
        apartment = data['apartment']
        if apartment.owner != user:
            raise serializers.ValidationError("You are not the owner of this apartment.")
        return data